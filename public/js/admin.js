...
console.log(urlFoto); // "" se for foto.png, ou o caminho se for outra
      ...
      set(newAtletaRef, {
        ...
        fotoPerfil.src = "/imagem/foto.png";
      }).catch((error) => {
...
// Excluir atletas
document.getElementById("excluir-atleta2-btn").onclick = () => {
...
// ===================== EXCLUIR CATEGORIA =====================
document.getElementById("excluir-categoria2-btn").onclick = () => {
...
categoriaOperador.addEventListener("change", () => {
  ...
  get(atletasRef).then(snapshot => {
    ...
    Object.entries(atletas).forEach(([id, atleta]) => {
      const fotoSegura = atleta.foto && atleta.foto.trim() !== "" ? atleta.foto : "/imagem/foto.png";
...
atletaOperador.addEventListener("change", () => {
  ...
  fotoAtleta.src = foto && foto.trim() !== "" ? foto : "/imagem/foto.png";
      ...
      get(ref(db3, `atletasPorCategoria/${categoria}/${id}`))
        .then(snapshot => {
          ...
          dadosParaEnviar[categoria][id] = {
            ...
            foto: foto && foto.trim() !== "" ? foto : "/imagem/foto.png",
            numero: numero
          };
