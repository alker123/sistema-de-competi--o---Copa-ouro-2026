...
function atualizarTabela() {
    ...
    if (!categoria) {
        ...
        tr.innerHTML = "<td colspan='7' class='empty-message'>Selecione uma categoria</td>";
    ...
    if (atletasDaCategoria.length > 0) {
        atletasDaCategoria.forEach(atletaNome => {
            ...
            const dados = dadosJogo[atletaNome] || { jogo1: false, jogo2: false, jogo3: false, jogo4: false }; 
            ...
            tr.innerHTML = `
                ...
                <td><input type="checkbox" class="jogo-checkbox" data-atleta="${atletaNome}" data-jogo="4" ${dados.jogo4 ? "checked" : ""} disabled></td>
                <td>${atletaNome}</td>
                <td>${categoria}</td>
            `;
            tabelaAtletas.appendChild(tr);
        ...
        const tr = document.createElement("tr");
        tr.innerHTML = "<td colspan='7' class='empty-message'>Nenhum atleta encontrado para esta categoria</td>";
        tabelaAtletas.appendChild(tr);
    }
}
                ...
                Object.keys(jogos).forEach(atletaNome => {
                    if (atletaNome in dadosAtletas) { // Verifica se o atleta existe na lista atual
                        ...
                        if (!dadosJogo[atletaNome]) {
                            dadosJogo[atletaNome] = { jogo1: false, jogo2: false, jogo3: false, jogo4: false };
                        }
...
btnEnviar.addEventListener("click", () => {
    ...
    const jogoSelecionado = ritmo === "1º Jogo" ? "1jogo" : ritmo === "2º Jogo" ? "2jogo" : ritmo === "3º Jogo" ? "3jogo" : "4jogo";
