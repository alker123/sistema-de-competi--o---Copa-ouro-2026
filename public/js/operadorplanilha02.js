...
const tabelaSecundaria2 = document.querySelector("#tabela-secundaria3");
...
const seletorCategoriaSecundaria = document.getElementById("seletor-categoria125");
const seletorFaseSecundaria = document.getElementById("seletor-fase-grupo2");
...
async function carregarDadosPorGrupoFaseSecundaria(fase) {
  const ritmos = ['1º Jogo', '2º Jogo', '3º Jogo', '4º Jogo'];
  const atletasNotas = {};
  ...
  for (let ritmoSelecionado of ritmos) {
    ...
    snap.forEach(child => {
      ...
      if (ritmoSelecionado === '4º Jogo') atletasNotas[atleta].media4ºJogo = mediaNum;
...
function exibirLinhasTabelaSecundaria(dados) {
  ...
  dadosFiltrados.sort((a, b) => {
    ...
    if (b.media3ºJogo !== a.media3ºJogo) {
      return b.media3ºJogo - a.media3ºJogo;
    }
    return b.media4ºJogo - a.media4ºJogo;
  });
  ...
  dadosFiltrados.forEach((dado, index) => {
    ...
    tr.innerHTML = `
      ...
      <td>${(dado.media4ºJogo ?? 0).toFixed(1)}</td>
    ...
    checkbox.addEventListener('change', async (e) => {
      ...
      if (atletaData) {
        if (isChecked) {
          ...
          if (atletaData.media4ºJogo < 21.0) {
            atletaData.media4ºJogo = 21.0;
          }
        } else {
          // Desmarca o atleta e volta para 20.0
          atletaData.media4ºJogo = 20.0;
        }
...
async function atualizarDadosFirebase(atletaData) {
  const categoriaSelecionada = seletorCategoriaSecundaria.value;
  const refAtletas = ref(db3, `medias/4º Jogo/${categoriaSelecionada}/${atletaData.atleta}`);
  ...
  await set(refAtletas, {
    ...
    media4ºJogo: atletaData.media4ºJogo,
