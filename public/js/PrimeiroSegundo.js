...
function segundo() {
    const fase = document.getElementById("seletor-fase-grupo2").value;  // Pega a fase selecionada
    const categoria = document.getElementById("seletor-categoria125").value;  // Pega a categoria selecionada
    ...
    const tabela = document.getElementById("tabela-secundaria3").getElementsByTagName('tbody')[0];
    ...
    function extrairDados(linha) {
        ...
        const jogo4 = colunas[8].textContent.trim();  // Nota 4º Jogo
        const notaFinal = colunas[9].textContent.trim();  // Nota Final
        const classificado = colunas[10].textContent.trim();  // Classificado para próxima fase

        return { posicao, foto, nomeAtleta, jogo1, jogo2, jogo3, jogo4, notaFinal, classificado };
    }
